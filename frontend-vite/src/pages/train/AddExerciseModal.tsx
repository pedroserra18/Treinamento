...
export function AddExerciseModal({
  ...
  // Recentes vêm do localStorage (alimentado pelo TrainPage quando
  // adiciona exercício no treino ativo).
  ...
  // (sem mostrar Recentes — evita duplicata). Filtros chip (músculo
  // / equipamento) só estreitam a lista de Recentes/Resultados,
  // não disparam o modo busca.
            ...
            <>
              {searchActive ? (
                ...
                <section>
                  ...
                  {/* Fallback: quando o usuário ainda não tem nada em
                      Recentes (primeiro uso), exibe todo o catálogo
                      filtrado pra ele não ficar sem opção. */}
                  {recent.length === 0 && !hasOnlyChipFilter && filtered.length > 0 && (
                    <>
                      <h3 className="bg-[var(--surface-hover)] px-4 py-2 text-[12px] font-bold uppercase tracking-wider text-[var(--muted)]">
                        Todos os Exercícios ({filtered.length})
                      </h3>
                      {filtered.map(renderRow)}
                    </>
                  )}
