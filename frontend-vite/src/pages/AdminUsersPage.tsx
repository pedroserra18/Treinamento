...
import { Ban, ChevronLeft, ChevronRight, Crown, Search, Trash2 } from 'lucide-react'
...
// Texto relativo curto ("há 2 dias", "hoje") para a coluna de datas.
...
// Gradiente do avatar determinístico por id (decorativo, fixo nos dois temas).
...
// Anima um número de 0 até o alvo (efeito count-up do cabeçalho).
...
export function AdminUsersPage() {
  ...
  const summary = data?.summary ?? { realCount: 0, testCount: 0, totalCount: 0 }
  const scopeLabel =
    accountScope === 'REAL' ? 'somente reais' : accountScope === 'TEST' ? 'somente teste' : 'reais + teste'

  const handleDeactivate = async (target: AdminUser) => {
    if (!window.confirm(`Deseja desativar a conta de ${target.email}?`)) return
    try {
      await deactivateUserByAdmin(authorizedFetch, target.id)
      await refresh()
    } catch (err) {
      window.alert(err instanceof Error ? err.message : 'Erro ao desativar usuario')
    }
  }
  ...
  const handleDelete = async (target: AdminUser) => {
    if (
      !window.confirm(
        `Deseja excluir a conta de ${target.email}? Esta acao remove a conta da listagem ativa.`,
      )
    )
      return
    try {
      await deleteUserByAdmin(authorizedFetch, target.id)
      await refresh()
    } catch (err) {
      window.alert(err instanceof Error ? err.message : 'Erro ao excluir usuario')
    }
  }
  ...
  return (
    <section className="space-y-3.5">
        ...
        <div className="relative z-[1] flex flex-col items-start justify-between gap-6 md:flex-row md:items-end">
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="text-right">
              ...
              <div className="mt-1 font-mono text-[10px] font-semibold text-emerald-600 dark:text-emerald-400">▲ esta semana</div>
            </div>
            <div className="text-right">
            ...
            <div className="text-right">
              ...
              <div className="mt-1 font-mono text-[10px] font-semibold text-emerald-600 dark:text-emerald-400">base completa</div>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="overflow-hidden rounded-2xl border border-[var(--line)] bg-[var(--surface)]">
        ...
        {loading ? (
          <p className="px-4 py-8 text-center text-sm text-[var(--muted)]">Carregando usuários…</p>
        ) : error ? (
                    ...
                    >
                      ...
                      <td className="!pl-4">
                        <div className="flex items-center gap-3">
                          <div
                            className="relative grid h-9 w-9 flex-shrink-0 place-items-center rounded-full text-[13px] font-semibold text-white"
        ...
        {/* Footer */}
        {!loading && !error ? (
          <div className="flex items-center justify-between border-t border-[var(--line)] bg-[var(--surface-hover)] px-4 py-3.5 text-xs text-[var(--muted)]">
            <span>
              Mostrando <b className="text-[var(--text)]">{filteredItems.length}</b> de{' '}
              <b className="text-[var(--text)]">{data?.total ?? 0}</b> usuários
            </span>
            <div className="inline-flex items-center gap-1.5">
              <button className="grid h-[30px] w-[30px] place-items-center rounded-lg border border-[var(--line)] bg-[var(--surface)] text-[var(--muted)] opacity-40" disabled aria-label="Anterior">
                <ChevronLeft size={12} />
              </button>
              <span className="grid h-[30px] w-[30px] place-items-center rounded-lg border border-[var(--brand)] bg-[var(--brand)] font-mono text-[11px] font-semibold text-white">
                1
              </span>
              <button className="grid h-[30px] w-[30px] place-items-center rounded-lg border border-[var(--line)] bg-[var(--surface)] text-[var(--muted)] opacity-40" disabled aria-label="Próxima">
